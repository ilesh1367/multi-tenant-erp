      ...
      <div className="bg-white dark:bg-slate-800 rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-2xl max-h-[92vh] sm:max-h-[85vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-start gap-3 sm:gap-4 px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-100">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-1">
              <span className="text-2xs font-bold px-2 py-0.5 rounded-full uppercase tracking-wider bg-slate-100 text-slate-600">
                {circular.target_audience_display || "All"}
              </span>
            </div>
            <h2 className="text-sm sm:text-base font-bold text-slate-800 leading-snug break-words">
              {circular.title}
            </h2>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 text-slate-500 flex-shrink-0 transition-colors"
          >
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 sm:py-5 space-y-4 sm:space-y-5">
          ...
          {circular.attachment_name && (
            <div>
              <p className="text-2xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                Attachment
              </p>
              <div className="flex items-center gap-3 bg-slate-50 rounded-xl px-3 sm:px-4 py-3 flex-wrap sm:flex-nowrap">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center gap-1 text-xs font-semibold text-blue-600 hover:underline flex-shrink-0 w-full sm:w-auto justify-center sm:justify-start"
                >
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-100 flex justify-end">
          <button
            onClick={onClose}
            className="w-full sm:w-auto px-5 py-2 bg-blue-600 text-white text-sm font-semibold rounded-xl hover:opacity-90 transition-opacity"
          >
            Close
          </button>
        </div>
...
function CircularCard({ circular, onClick }) {
  ...
  return (
    <div
      onClick={onClick}
      className="bg-white rounded-xl p-3.5 sm:p-4 lg:p-5 shadow-sm border border-slate-100
      ...
      <div className="flex items-start gap-3 sm:gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-start justify-between gap-1.5 sm:gap-2 mb-1">
            <h3 className="text-sm font-bold text-slate-800 leading-snug group-hover:text-blue-700 transition-colors pr-0 sm:pr-2 break-words">
              {circular.title}
            </h3>
            <span className="text-2xs font-bold px-2 py-0.5 rounded-full uppercase tracking-wider flex-shrink-0 bg-slate-100 text-slate-600 self-start">
              {circular.target_audience_display || "All"}
            </span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5">
            ...
            {circular.attachment_name && (
              <span className="flex items-center gap-1 text-2xs text-blue-600 font-semibold">
                <span className="material-symbols-outlined text-sm">attach_file</span>
                Attachment
              </span>
            )}
            <span className="w-full sm:w-auto sm:ml-auto text-2xs font-semibold text-blue-600 flex items-center gap-0.5 group-hover:gap-1 transition-all">
              Read more
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm
                       focus:outline-none focus:ring-2 focus:ring-blue-200 text-slate-700
